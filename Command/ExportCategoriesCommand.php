...
use LizardsAndPumpkins\Magento2Connector\Model\CategoryExport\CategoryCollector;
use LizardsAndPumpkins\Magento2Connector\Model\CategoryExport\CategoryListXmlExporter\CategoryListXmlExporterList;
use LizardsAndPumpkins\Magento2Connector\Model\CategoryExport\CategoryListXmlExporter\CategoryListXmlToFileExporter;
...
class ExportCategoriesCommand extends Command
{
    ...
    /**
     * @var CategoryListXmlExporterList
     */
    ...
    public function __construct(
        CategoryListXmlExporterList $exporterList,
    ...
    {
        $this
            ->setName('lizardsandpumpkins:categories:export')
            ->setDescription('Export all Categories')
            ->addOption(
                ...
                'b',
                InputOption::VALUE_OPTIONAL,
                'Maximum number of categories per XML',
                100
            )->addOption(
                ...
                CategoryListXmlToFileExporter::TYPE
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        do {
            $categoryCollection = $this->categoryCollector->getCollection($store, $pageSize, $page);
            $result = $exporter->exportCategoryListXml($categoryCollection->getItems(), $context);
            $output->writeln($result->getMessages());
        } while (false === $this->categoryCollector->shouldCancel($categoryCollection, $pageSize, $page++));
    }
}
